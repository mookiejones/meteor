...
Template.workersListPage.events({
  // use keyup to implement dynamic filtering
  // keyup is preferred to keypress because of end-of-line issues
  'keyup #workersSearchInput': function() {
    Session.set('accountSearchFilter', $('#workersSearchInput').val());
  },
  ...
  // Template.workersListPage.paginationButtonList()
...
Template.workersListPage.helpers({
  workersList: function() {
    ...
    Session.set('paginationCount', Math.floor(WorkerAccounts.find().count() / Session.get('tableLimit')));

    var reg = Session.get('accountSearchFilter');
    console.log(reg);

    // this is a performant local (client-side) search on the data
    // current in our workerAccounts cursor, and will reactively
    // update the table
  ...
  rendered: function() {
    ...
    $(this.find('#workersTable')).tablesorter();
